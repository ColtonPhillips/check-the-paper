...
DEFAULT_OUTPUT_FILE = os.path.join("..", "src", "Assets.as")
...
def chince_function(l):
    s = "\t"
    s += "public static var chinceArray:Array = ["
    for i in l[0:-1]:
        s += i
        s += ', '
    s += l[-1]
...
""");
    ...
    for extension, files_with_extension in asset_dict.items():
        ...
        for file_path in files_with_extension:
            ...
            if (extension != 'png' and extension != 'mp3' and extension != 'jpg'):
                file.write(", mimeType = \"application/octet-stream\"")
            
            file.write(")] public static const " + asset_name + ":Class\n")
            chince_list.append(asset_name)
 
        if (extension != 'ttf'):
            file.write("\n")
 
    file.write(chince_function(chince_list))
    file.write("\t}\n}")
    file.close()
